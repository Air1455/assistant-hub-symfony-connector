...
final readonly class AuthorizationController
{
    ...
    public function authorize(Request $request): Response
    {
        ...
        if ($this->authorization->requiresCredentials()) {
            return $this->loginPage($csrf, authorization: $authorization);
        }

        try {
            $session->set(self::SESSION_VAULT, $this->authorization->connect());
        } catch (ProtocolException $exception) {
            return $this->page('Connexion au site requise', '<p class="error">'.$this->escape($exception->getMessage()).'</p>', $exception->httpStatus);
        }

        return $this->consentPage($csrf, $authorization, false);
    }
    ...
    {
        if (!$this->authorization->requiresCredentials()) {
            return $this->page('Connexion déjà gérée par le site', '<p class="error">Utilisez la session sécurisée du site.</p>', 404);
        }
        if (!$this->validCsrf($request)) {
            return $this->page('Session expirée', '<p class="error">Rechargez la demande depuis le Hub.</p>', 400);
        }
        try {
            $vaultId = $this->authorization->connect($request->request->getString('username'), $request->request->getString('password'));
            $request->getSession()->set(self::SESSION_VAULT, $vaultId);
            $request->request->set('password', '');
        } catch (ProtocolException $exception) {
            return $this->loginPage((string) $request->getSession()->get(self::SESSION_CSRF), $exception->getMessage(), 401, $request->getSession()->get(self::SESSION_AUTHORIZATION));
        }
        ...
        return $this->consentPage((string) $request->getSession()->get(self::SESSION_CSRF), $auth, true);
    }
    ...
    {
        try {
            $pair = $this->authorization->exchange($request->toArray());
    ...
    private function consentPage(string $csrf, array $authorization, bool $apiTokens): Response
    {
        $hubOrigin = is_string($authorization['redirect_uri'] ?? null)
            ? (string) parse_url($authorization['redirect_uri'], PHP_URL_HOST)
            : 'Assistant Hub';
        $trust = $apiTokens
            ? 'Compte authentifié par l’API officielle de '.$this->connectorName
            : 'Compte authentifié par la session sécurisée de '.$this->connectorName;
        $security = $apiTokens
            ? '<li>Les jetons restent chiffrés dans le coffre du connecteur.</li><li>Chaque demande restera contrôlée par l’API du site.</li>'
            : '<li>Aucun cookie de session ni mot de passe n’est copié dans le connecteur.</li><li>Chaque demande restera soumise aux autorisations locales du site.</li>';
        $content = '<div class="trust"><strong>Connexion vérifiée</strong><span>'.$this->escape($trust).'</span></div>'
            .'<p>Assistant Hub demande à relier ce compte au connecteur <strong>'.$this->escape($this->connectorName).'</strong>.</p>'
            .'<ul><li>Le Hub ne recevra jamais votre mot de passe.</li>'.$security.'</ul>'
            .'<p class="muted">Demande initiée par '.$this->escape($hubOrigin).'. Cette autorisation doit être confirmée dans les 10 minutes.</p>'
